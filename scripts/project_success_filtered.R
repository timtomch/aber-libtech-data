...
plotspath <- "plots-filtered"

# Run project_success.R but only on projects that were procured through RFP.
...
## Removing the negotiation skill because all RFP responses have this skill represented.
...
skills_fit <- lm(delonemclean ~ proj_mgmt_rep + accounting_rep + communications_rep +
                   accessibility_rep + UX_rep + design_rep +
...
print('@@@@@@           END FILTER                @@@@@@')
# Restore full survey set
survey <- unfiltered_survey
